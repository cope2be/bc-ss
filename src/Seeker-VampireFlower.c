...
// Modifications:
//  made the program single threaded
//  commented out stdio and stdlib dependencies
//  changed main fn into seek_seed fn
//  added result getter fns

// feel free to correct me if i got the licensing wrong
...
Cat all_cats[MAX_CATS];

uint USER_NCATS;
Cat* cats = all_cats; // globally accessible pointer to array of cats
...
FORCE_INLINE bool verify_seed(uint seed) {
    ...
    for (uint j = 0; j < USER_NCATS; j++) {
        ...
        rarity pulled_rarity = (rarity)((temp >= SUPER_CHANCE)
                             + (temp >= UBER_CHANCE)
                             + (temp >= LEGEND_CHANCE));
...
FORCE_INLINE void set_start_and_end(/*ThreadArgs* arg*/bool method, uint low, uint high) {

    // uint thread_id = arg->thread_id;
    // uint low   = arg->low;
    // uint high  = arg->high;
    uint new_start = (/*UINT32_MAX / thread_count) * thread_id*/0);
    uint new_end   = (UINT32_MAX/* / thread_count) * (thread_id + 1*/);

    if (/*arg->*/method) { // rarity method

        if (mod_10000(new_start) < high) { // we landed before/in a range, move to its start.
            new_start -= mod_10000(new_start); // set lowest 4 digits to 0
            new_start += low;
        } else { // we landed after a range, move to the next one.
            new_start -= mod_10000(new_start);
            new_start += low + 10000;
        }
        ...
        // else // assign all the leftover values to the last thread
            new_end = (high > 7295) ? UINT32_MAX : 4294960000 + high;
    }
        ...
        // else // last thread special case
            new_end = UINT32_MAX - ((UINT32_MAX - b) % m);
...
void* find_seed_fast(/*void* args*/bool method, uint low, uint high) {
    // ThreadArgs* arg = (ThreadArgs*)args;

    // determine the range of values this thread should process
    set_start_and_end(/*arg*/method, low, high);
...
void run_search_threads(
    ...
    // for (uint i = 0; i < thread_count; i++) {
        ...
        find_seed_fast(method, low, high);
    // }
...
void seek_seed(
    uint rare_chance, uint supa_chance, uint uber_chance,
    uint rare_amount, uint supa_amount, uint uber_amount, uint legend_amount,

    // lol
    uint cat_amount,
    uint slot_0, uint rarity_0,
    uint slot_1, uint rarity_1,
    uint slot_2, uint rarity_2,
    uint slot_3, uint rarity_3,
    uint slot_4, uint rarity_4,
    uint slot_5, uint rarity_5,
    uint slot_6, uint rarity_6,
    uint slot_7, uint rarity_7,
    uint slot_8, uint rarity_8,
    uint slot_9, uint rarity_9
    ) {
    ...
    LEGEND_CHANCE = /*10000 - atoi(argv[5])*/rare_chance + supa_chance + uber_chance;
    UBER_CHANCE = LEGEND_CHANCE - /*atoi(argv[4])*/uber_chance;
    SUPER_CHANCE = UBER_CHANCE - /*atoi(argv[3])*/supa_chance;
    RARITY_SIZES[0] = /*atoi(argv[6])*/rare_amount; // number of rare cats on the banner
    RARITY_SIZES[1] = /*atoi(argv[7])*/supa_amount; // number of super rare cats on the banner
    RARITY_SIZES[2] = /*atoi(argv[8])*/uber_amount; // number of uber cats on the banner
    RARITY_SIZES[3] = /*atoi(argv[9])*/legend_amount; // number of legend rare cats on the banner

    USER_NCATS = /*(argc - 10) / 2*/cat_amount; // cats start at argv 10
    ...
    // for (uint i = 0, j = 10; i < USER_NCATS && i < MAX_CATS; i++) {
    //     cats[i].rarity = atoi(argv[j++]);
    //     cats[i].slot   = atoi(argv[j++]);
    // }

    // i love hardcoding
    cats[0].rarity = (rarity)rarity_0;
    cats[0].slot = slot_0;
    cats[1].rarity = (rarity)rarity_1;
    cats[1].slot = slot_1;
    cats[2].rarity = (rarity)rarity_2;
    cats[2].slot = slot_2;
    cats[3].rarity = (rarity)rarity_3;
    cats[3].slot = slot_3;
    cats[4].rarity = (rarity)rarity_4;
    cats[4].slot = slot_4;
    cats[5].rarity = (rarity)rarity_5;
    cats[5].slot = slot_5;
    cats[6].rarity = (rarity)rarity_6;
    cats[6].slot = slot_6;
    cats[7].rarity = (rarity)rarity_7;
    cats[7].slot = slot_7;
    cats[8].rarity = (rarity)rarity_8;
    cats[8].slot = slot_8;
    cats[9].rarity = (rarity)rarity_9;
    cats[9].slot = slot_9;
